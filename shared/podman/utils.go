...
// ExecOnRunningContainer execute a command in a running container.
func ExecOnRunningContainer(
	name string, cmd []string,
) []string {
	podmanArgs := []string{"exec", "-ti", name}
	podmanArgs = append(podmanArgs, cmd...)
...
func RunningContainerInspect[T any](
	...
	args := ExecOnRunningContainer("uyuni-server", []string{"bash", "-c", script})
	out, err := newRunner("podman", args...).Log(zerolog.DebugLevel).Exec()
	if err != nil {
		return nil, err
	}
