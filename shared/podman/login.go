...
func PodmanLogin(hostData *HostInspectData, scc types.SCCCredentials) (string, func(), error) {
	...
	"auths": {
		...
		if err != nil {
			return "", nil, utils.Errorf(err, L("failed to set authentication for %s"), scc.Registry)
		}
		...
		if err := authFile.Close(); err != nil {
			...
			return "", nil, utils.Errorf(err,
				L("failed to close the temporary auth file. Cannot set authentication for %s"), scc.Registry)
		}
