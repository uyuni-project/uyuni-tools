...
func CreatePersistentVolumeClaims(
	...
	return Apply(pvcs, L("failed to run the persistent volume claims"))
}
...
func GetPersistentVolumeClaims(
	...
	for _, mount := range mounts {
		...
		pv := newPersistentVolumeClaim(
			namespace, mount.Name, storageClass, size, []core.PersistentVolumeAccessMode{accessMode}, matchPvByLabel,
		)
