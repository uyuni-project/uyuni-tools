...
func (*systemFetcher) Help() ResourceHelp {
	return ResourceHelp{
		Summary: L("List systems"),
		Details: L("Systems support server-side filtering, sorting, and pagination."),
		Examples: L(`  # List all systems
  mgrctl get system

  # Use the system alias
  mgrctl get sys

  # List systems that have extra packages installed
  mgrctl get system --filter "extra_pkg_count>0"

  # Sort systems by name
  mgrctl get system --sort-by name

  # Show the first page of ten systems
  mgrctl get system --page 0 --page-size 10`),
	}
}

func (*systemFetcher) List(client *api.APIClient, opts ListOptions) ([]System, int, error) {
	filterKey, filterValue := "", ""
	if opts.Filter != "" {
		var err error
		filterKey, filterValue, err = parseFilter(opts.Filter)
		if err != nil {
			return nil, 0, err
		}
	}
	...
	query.Set("page", fmt.Sprintf("%d", opts.PageNumber))
	query.Set("pageSize", fmt.Sprintf("%d", opts.PageSize))
	query.Set("sortKey", opts.SortBy)
	query.Set("sortDescending", "false")
...
func parseFilter(expr string) (string, string, error) {
	...
	if key == "" {
		return "", "", fmt.Errorf(L("filter key cannot be empty in %q"), expr)
	}

	for _, c := range key {
		if !unicode.IsLetter(c) && !unicode.IsDigit(c) && c != '_' && c != '-' {
			return "", "", fmt.Errorf(
				L("invalid filter key %q: only letters, digits, underscores and dashes are allowed"),
				key,
			)
		}
	}
