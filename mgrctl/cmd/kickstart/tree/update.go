...
func updateCommand(globalFlags *types.GlobalFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Edit a Kickstart Tree (Distribution) in #product().",
	...
	cmd.Flags().String("TreeLabel", "", "Label for the kickstart tree.")
	cmd.Flags().String("BasePath", "", "Path to the base or root of the kickstart tree.")
	cmd.Flags().String("ChannelLabel", "", "Label of channel to associate with kickstart tree.")
