...
func NewConfigCommand(globalFlags *types.GlobalFlags) *cobra.Command {
    ...
    $ mgrctl proxy create config --proxyName="proxy.example.com" --server="server.example.com" --email="admin@example.com" --caCrt="root_ca.pem" --proxyCrt="proxy_crt.pem" --proxyKey="proxy_key.pem"

  Create a proxy configuration file providing certificates providing  all parameters
	
    $ mgrctl proxy create config --proxyName="proxy.example.com" --server="server.example.com" --email="admin@example.com" --caCrt="root_ca.pem" --proxyCrt="proxy_crt.pem" --proxyKey="proxy_key.pem" --intermediateCAs="intermediateCA_1.pem,intermediateCA_2.pem,intermediateCA_3.pem" -o="proxy-config"
	
  or an alternative format:

    $ mgrctl proxy create config --proxyName="proxy.example.com" --server="server.example.com" --email="admin@example.com" --caCrt="root_ca.pem" --proxyCrt="proxy_crt.pem" --proxyKey="proxy_key.pem" --intermediateCAs "intermediateCA_1.pem" --intermediateCAs "intermediateCA_2.pem" --intermediateCAs "intermediateCA_3.pem" -o="proxy-config"
	
  Create a proxy configuration file with generated certificates providing only required parameters
	
    $ mgrctl proxy create config --proxyName="proxy.example.com" --server="server.example.com" --email="admin@org.com" --caCrt="ca.pem" --caKey="caKey.pem"
	
  Create a proxy configuration file with generated certificates providing only required parameters and ca password
	
    $ mgrctl proxy create config --proxyName="proxy.example.com" --server="server.example.com" --email="admin@org.com" --caCrt="ca.pem" --caKey="caKey.pem" --caPassword="pass.txt"

  Create a proxy configuration file with generated certificates providing all parameters
	
    $ mgrctl proxy create config --proxyName="proxy.example.com" --server="server.example.com" --email="admin@org.com" --caCrt="ca.pem" --caKey="caKey.pem" --caPassword="pass.txt" --cnames="proxy_a.example.com,proxy_b.example.com,proxy_c.example.com" --country="DE" --state="Bayern" --city="Nuernberg" --org="orgExample" --orgUnit="orgUnitExample" --sslEmail="sslEmail@example.com" -o="proxy-config"
...
func proxyCreateConfigInit(globalFlags *types.GlobalFlags, flags *ProxyCreateConfigFlags, cmd *cobra.Command, args []string) error {
...
func addFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringSliceP(intermediateCAs, "i", []string{}, L("Path to an intermediate CA used to sign the proxy certicate in PEM format. May be provided multiple times or separated by commas."))
	...
	cmd.Flags().String(caPassword, "", L("Path to a file containing the password of the CA private key, will be prompted if not passed."))
	cmd.Flags().StringSlice(cNames, []string{}, L("Alternate name of the proxy to set in the certificate. May be provided multiple times or separated by commas."))
	...
	_ = utils.AddFlagsToHelpGroupID(cmd, generateGroup, caKey, caPassword, cNames, country, state, city, org, orgUnit, sslEmail)
}
