...
func ptfForPodman(
	...
	httpdImage := getImage(authFile, flags.UpgradeFlags.Httpd.Name, pullPolicy, "httpd")
	saltBrokerImage := getImage(authFile, flags.UpgradeFlags.SaltBroker.Name, pullPolicy, "salt-broker")
	squidImage := getImage(authFile, flags.UpgradeFlags.Squid.Name, pullPolicy, "squid")
	sshImage := getImage(authFile, flags.UpgradeFlags.SSH.Name, pullPolicy, "ssh")
	tftpdImage := getImage(authFile, flags.UpgradeFlags.Tftpd.Name, pullPolicy, "tftpd")
...
func getImage(authFile string, image string, policy string, service string) string {
	var newImage string
	var err error
	if image != "" {
