...
func GenerateSystemdService(
...

	{
		...
		additionHTTPConfPath, err := getFilePath(flags.ProxyImageFlags.Tuning.Httpd, defaultApacheConf)
		if err != nil {
			return err
		}
	...
	// Squid
	{
		...
		additionSquidConfPath, err := getFilePath(flags.ProxyImageFlags.Tuning.Squid, defaultSquidConf)
		if err != nil {
			return err
		}
...
func getFilePath(flag string, defaultPath string) (string, error) {
	result := ""
	if flag != "" {
		var err error
