...
func Upgrade(
	...
	if eventProcessorFlags.Image.Name == "" {
		return errors.New(L("image is required"))
	}

	if err := writeEventProcessorFiles(
		systemd, authFile, eventProcessorFlags, baseImage, db, debugJava,
	); err != nil {
		return err
	}
...
func writeEventProcessorFiles(
	...
	eventProcessorImage, err := utils.ComputeImage(baseImage.Registry.Host, baseImage.Tag, image)
	if err != nil {
