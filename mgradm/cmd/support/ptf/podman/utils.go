...
func (flags *podmanPTFFlags) checkParameters() error {
	...
	flags.Image.Name, err = utils.ComputePTF(flags.SCC.Registry, flags.CustomerID, projectID, serverImage, suffix)
	if err != nil {
		return err
	}
	...
	for service, pointer := range images {
		if containerImage := getServiceImage(service); containerImage != "" {
			...
			containerImage, err = utils.ComputePTF(flags.SCC.Registry, flags.CustomerID, projectID, containerImage, suffix)
			if err != nil {
				return err
			}
