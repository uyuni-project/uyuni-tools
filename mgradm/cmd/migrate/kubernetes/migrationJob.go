...
func startMigrationJob(
	...
	// Run the job
	return kubernetes.Apply([]runtime.Object{job}, L("failed to run the migration job"))
}
...
func getMigrationJob(
	...
	job, err := kubernetes.GetScriptJob(namespace, migrationJobName, image, pullPolicy, mounts, scriptData)
	if err != nil {
		return nil, err
	}
